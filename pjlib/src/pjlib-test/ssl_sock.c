...
int ssl_sock_test(void)
{
    ...
    ret = echo_test(PJ_SSL_SOCK_PROTO_TLS1, PJ_SSL_SOCK_PROTO_TLS1,
                    PJ_TLS_RSA_WITH_3DES_EDE_CBC_SHA, PJ_TLS_RSA_WITH_3DES_EDE_CBC_SHA,
                    PJ_FALSE, PJ_FALSE);
    if (ret != 0)
        return ret;
return 0;
    ...
    ret = echo_test(PJ_SSL_SOCK_PROTO_TLS1, PJ_SSL_SOCK_PROTO_SSL3, 
		    PJ_TLS_DHE_DSS_WITH_3DES_EDE_CBC_SHA, PJ_TLS_DHE_DSS_WITH_3DES_EDE_CBC_SHA,
		    PJ_FALSE, PJ_FALSE);
    if (ret == 0)
	return PJ_EBUG;
    ...
    ret = echo_test(PJ_SSL_SOCK_PROTO_DEFAULT, PJ_SSL_SOCK_PROTO_DEFAULT, 
		    PJ_TLS_DHE_DSS_WITH_3DES_EDE_CBC_SHA, PJ_TLS_RSA_WITH_AES_256_CBC_SHA,
		    PJ_FALSE, PJ_FALSE);
    if (ret == 0)
	return PJ_EBUG;
